...
validate_pd_topology() {
    ...
    if [[ "${P_INDEPENDENT_NODE_DP:-0}" == "1" ]]; then
        # Each Prefill node is its own DP group (DeepSeek 2P1D style).
        if (( P_DP_SIZE != P_DP_SIZE_LOCAL )); then
            echo "[topology] P_INDEPENDENT_NODE_DP=1 requires P_DP_SIZE(${P_DP_SIZE}) == P_DP_SIZE_LOCAL(${P_DP_SIZE_LOCAL})" >&2
            ok=1
        fi
    else
...
print_pd_topology() {
    ...
    echo "  1) Edit IP-aligned lists in configs_*.sh (IPS/NICS/devices/ports)"
    ...
    echo "  independent_node_dp=${P_INDEPENDENT_NODE_DP:-0}"
    echo "  pp_layer_partition=${P_PP_LAYER_PARTITION:-none}"
    ...
    for i in "${!PREFILL_IPS[@]}"; do
        ...
        if [[ "${P_INDEPENDENT_NODE_DP:-0}" == "1" ]]; then
            echo "         ranks 0..$((P_DP_SIZE_LOCAL - 1)) (own DP group, dp_address=${PREFILL_IPS[$i]})"
        else
    ...
    echo "  pp_layer_partition=${D_PP_LAYER_PARTITION:-none}"

...
# Bootstrap COMMON_DIR (prefer shared mount of common_v2) before sourcing configs.
_ENTRY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SHARED_COMMON_DIR="${SHARED_COMMON_DIR:-/mnt/a800_share/l00848175/scripts-ascend/example/common_v2}"
if [[ -d "${SHARED_COMMON_DIR}" && -f "${SHARED_COMMON_DIR}/load_configs.sh" ]]; then

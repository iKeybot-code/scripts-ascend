...
resolve_common_dir() {
    ...
    local shared_default="/mnt/a800_share/l00848175/scripts-ascend/example/common"
    ...
    if [[ -f "${scenario_dir}/../common/config_helpers.sh" ]]; then
        export COMMON_DIR="$(cd "${scenario_dir}/../common" && pwd)"
        return 0
    fi
    ...
    echo "[resolve_common_dir] cannot locate common/" >&2
    echo "  Options:" >&2
    echo "    export COMMON_DIR=/path/to/common" >&2
    echo "    export SHARED_COMMON_DIR=/path/to/common" >&2
    echo "    keep ../common next to the scenario, or copy common/ inside the scenario" >&2
    return 1
}

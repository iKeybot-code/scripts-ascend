...
### 6.5 AscendStore：远程 KV Cache Pool
...
- `vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/backend/*.py`

**改造内容**：

1. **多后端支持**：通过 `backend_map` 动态加载 Mooncake、Memcache、元戎等存储后端，统一抽象为 `Backend` 接口（`batch_copy`/`put`/`get`/`exists`）。
2. **Scheduler 侧命中检测**：
   - `KVPoolScheduler.get_num_new_matched_tokens` 通过 ZMQ RPC (`LookupKeyClient`) 或直连后端进行前缀命中检测。
   - 支持 layerwise 模式下的 GVA 命中检测 (`_get_layerwise_gva_hit_tokens`)。
   - 支持 Hybrid KV Cache（如 FullAttention + Mamba）多 group 协调命中，取各 group 最小公共命中长度。
3. **Worker 侧 KV 加载/保存**：
   - `KVPoolWorker.start_load_kv` 根据 `LoadSpec` 异步或同步从远程 Pool 加载 KV block。
   - `KVPoolWorker.wait_for_save` 将本地 KV 通过发送线程保存到远程 Pool。
   - `register_kv_caches` 时解析 K/V 分离 tensor，记录每层 base addr / block stride，并启动传输线程。
4. **传输线程模型**：
   - `KVCacheStoreSendingThread` / `KVCacheStoreRecvingThread`：非 layerwise 全量 KV 传输。
   - `KVCacheStoreLayerSendingThread` / `KVCacheStoreLayerRecvingThread`：layerwise GVA 路径，逐层 batch_copy，支持与计算流水线重叠。
   - `KVCacheStoreKeyLayerSendingThread` / `KVCacheStoreKeyLayerRecvingThread`：layerwise Key 路径，逐层 put/get。
5. **多 group / Hybrid 支持**：
   - `KVPoolWorker` 支持多 `kv_cache_group`，每个 group 独立配置 block_size、cache_family、layer mapping。
   - `_build_group_layer_builders` 为每个 group 创建独立的 `LayerBatchBuilder`。
6. **GVA (Global Virtual Address) 支持**：
   - Memcache 后端支持 GVA 模式，通过 `batch_alloc` 预分配全局虚拟地址，`batch_copy` 实现零拷贝传输。
   - Worker 侧 `_alloc_gvas_for_save` 在保存前预分配 GVA；`_prepare_load_gvas` 在加载前获取 GVA 并加读租约。
7. **TP Mismatch 支持**：
   - 当本地 TP size 与远端不匹配时，`_build_strided_addrs` 按 head 维度拆分，实现 strided I/O。
   - `_store_kv_tp_mismatch` / `_load_kv_tp_mismatch` 处理 mismatched TP 的保存与加载。
8. **异步加载**：
   - `load_async=True` 时，`KVCacheStoreRecvingThread` 在后台持续接收 KV，不阻塞 `execute_model`。
   - Scheduler 通过 `_loading_req_ids` 跟踪异步加载状态。

### 6.6 RecomputeCPUOffload：重计算场景下的 CPU Offload

**文件**：

- `vllm_ascend/distributed/kv_transfer/kv_pool/recompute_cpu_offload/recompute_cpu_offload_connector.py`
...
1. **重计算保护机制**：当请求被抢占时，将其 KV 临时 offload 到 CPU pinned memory，恢复后再通过 NPU stream 传回 HBM。
2. **Scheduler 侧**：`RecomputeCPUOffloadScheduler` 管理 CPU 容量配额、block 分配、prefix caching 命中检测。
3. **Worker 侧**：`RecomputeCPUOffloadWorker` 使用 `torch.npu` stream 和 `aclrtMemcpyBatchAsync` 执行 H2D/D2H 异步拷贝。
4. **状态同步**：`handle_preemptions` 在请求被抢占前触发保存；`start_load_kv` 在恢复时触发加载。
...
| **AscendStore Connector** | `vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/ascend_store_connector.py` | 远程 KV Pool 连接器，封装 Scheduler/Worker 双端接口 | 否 |
| **AscendStore Worker** | `vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/pool_worker.py` | KV Pool Worker 侧缓存引擎，加载/保存/GVA/TP mismatch | 否 |
| **AscendStore Scheduler** | `vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/pool_scheduler.py` | KV Pool Scheduler 侧调度器，命中检测/Metadata 构建 | 否 |
| **AscendStore Transfer** | `vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/kv_transfer.py` | 传输线程实现（全量/Layerwise/GVA/Key 路径） | 否 |
| **AscendStore Backend** | `vllm_ascend/distributed/kv_transfer/kv_pool/ascend_store/backend/*.py` | Mooncake/Memcache/元戎后端适配 | 否 |
| **RecomputeCPUOffload Connector** | `vllm_ascend/distributed/kv_transfer/kv_pool/recompute_cpu_offload/recompute_cpu_offload_connector.py` | 重计算 CPU Offload 连接器 | 否 |
| **RecomputeCPUOffload Worker** | `vllm_ascend/distributed/kv_transfer/kv_pool/recompute_cpu_offload/worker.py` | 重计算 Worker 侧异步拷贝实现 | 否 |
| **RecomputeCPUOffload Manager** | `vllm_ascend/distributed/kv_transfer/kv_pool/recompute_cpu_offload/manager.py` | 重计算 Scheduler 侧容量与状态管理 | 否 |
...
### 8.4 AscendStore KV Pool 侧（vLLM-Ascend 实现）

| 方法 | 所属类 | 功能 |
|------|--------|------|
| `get_num_new_matched_tokens` | `KVPoolScheduler` | Scheduler 侧检测远程 Pool 前缀命中长度 |
| `build_connector_meta` | `KVPoolScheduler` | 构建包含 load_spec / block_ids 的 AscendConnectorMetadata |
| `update_state_after_alloc` | `KVPoolScheduler` | 分配 block 后启用 load_spec.can_load |
| `request_finished` / `request_finished_all_groups` | `KVPoolScheduler` | 请求结束时决定 block 是否延迟释放 |
| `lookup_scheduler` | `KVPoolWorker` | Worker 侧通过 ZMQ 响应 Scheduler 的命中查询 |
| `register_kv_caches` | `KVPoolWorker` | 解析 K/V tensor 布局，注册后端 buffer，启动传输线程 |
| `start_load_kv` | `KVPoolWorker` | 根据 metadata 启动 KV 加载（同步或异步） |
| `wait_for_save` | `KVPoolWorker` | 将待保存请求投递到发送线程队列 |
| `get_finished` | `KVPoolWorker` | 获取已完成 send/recv 的请求集合 |
| `process_layer_data` | `KVPoolWorker` | layerwise 模式下预计算各层的 save/load task |
| `_alloc_gvas_for_save` | `KVPoolWorker` | GVA 模式下预分配全局虚拟地址 |
| `_prepare_load_gvas` | `KVPoolWorker` | GVA 模式下获取 GVA 并添加读租约 |
| `_batch_copy_with_limits` | `KVTransferThread` | 调用后端 batch_copy，支持分块和限流 |

### 8.5 RecomputeCPUOffload 侧（vLLM-Ascend 实现）

| 方法 | 所属类 | 功能 |
|------|--------|------|
| `handle_preemptions` | `RecomputeCPUOffloadWorker` | 请求被抢占前将 KV offload 到 CPU |
| `start_load_kv` | `RecomputeCPUOffloadWorker` | 恢复时将 KV 从 CPU 传回 NPU |
| `get_num_new_matched_tokens` | `RecomputeCPUOffloadScheduler` | 检测 CPU 中是否有可复用的 KV prefix |
| `update_state_before_preempt` | `RecomputeCPUOffloadScheduler` | 抢占前决定是否保存及更新状态 |
| `has_pending_transfers` | `RecomputeCPUOffloadConnectorV1` | 检查是否有未完成的 CPU-NPU 传输 |
...
AscendStore 支持多种后端（Mooncake、Memcache、元戎），不同后端在错误码、重试策略、租约语义上存在差异：

- **Mooncake**：`put/get` 返回错误码需手动映射到 `invalid_block_ids`；网络抖动时可能导致整批 block 加载失败。
- **Memcache GVA**：`batch_alloc` 非幂等，重复分配同一 key 会返回 `MMC_DUPLICATED_OBJECT`；Worker 必须通过 `_allocated_gvas` 缓存已分配的 GVA，避免重复分配。
- **元戎后端**：目前 layerwise 和 GVA 模式的支持程度需要验证。

### 9.6 Layerwise 传输与计算流水线的同步点

Layerwise 模式通过 `layer_load_finished_events` / `layer_save_finished_events` 在传输线程与 Model Runner 之间同步：

- `wait_for_layer_load` 使用 10 秒超时，若后端延迟过高会导致加载失败、推理回退到重新计算。
- `sync_save_events[layer_id].synchronize()` 在保存前强制同步 NPU stream，可能破坏 V2 的 async-first 设计。
- 需要审计 `attention_compute_start_gate` 的时序，确保 layerwise 加载不会超前于 attention 计算。

### 9.7 Hybrid KV Cache 多 group 一致性

当模型同时存在 FullAttention 和 Mamba（或 SWA）group 时：

- 各 group 的 `block_size`、`cache_family`、`transfer_granularity` 不同，Scheduler 侧取 `min(hits)` 作为最终命中长度。
- 若某一 group 的 KV 加载失败（如后端不存在），当前实现会跳过 `invalid_block_ids` fallback 以避免 scheduler crash，但这可能导致该请求的部分 token 无 KV 可用。
- Mamba group 的 `align` 模式要求 block_id > 0 时才参与传输，`_skip_null_blocks` 逻辑需要与 scheduler 保持一致。

### 9.8 TP Mismatch 的兼容性限制

TP mismatch 当前存在以下限制：

- 不支持 sparse KV layout（`use_sparse=True`）。
- 不支持 layerwise 传输。
- 不支持 Hybrid KV Cache（多 group）。
- 这些限制在 `_init_key_head_config` 中通过显式 `raise` 保护，但未来若需支持需重新设计 strided I/O 路径。

### 9.9 RecomputeCPUOffload 的内存与同步风险

- CPU 容量按 `cpu_bytes_to_use / world_size` 均分，若各 rank 的 KV 大小不均可能导致某些 rank 提前耗尽 CPU 内存。
- `handle_preemptions` 和 `start_load_kv` 均使用 NPU stream 异步拷贝，但若调度器在传输完成前再次抢占同一请求，可能导致数据竞争。
...
- **真正需要持续维护的改造点在 connector 内部和 KV cache 分配 patch**：
  ...
  - Async 调度下的同步点控制。
- **KV Pool 池化是 vLLM-Ascend 的核心扩展能力**：
  - AscendStore 提供远程 KV Cache Pool，支持多后端、多 group、layerwise 流水线、GVA 零拷贝；
  - RecomputeCPUOffload 提供重计算保护，避免抢占导致的重复计算；
  - 这些 Pool 化能力在 MRV2 下通过 V1 基类 + ActiveKVConnector 桥接即可工作，无需改动 model runner。

...
vllm serve /mnt/weight/DeepSeek-V3.1-w4a8-mtp-QuaRot \
  --host 0.0.0.0 --port "${2:-8001}" \
  --data-parallel-size 1 \
  --tensor-parallel-size 8 --enable-expert-parallel \
  --seed 1024 --served-model-name deepseek_v3 \
  --max-model-len 65536 --max-num-batched-tokens 256 --max-num-seqs 28 \
  --enforce-eager --trust-remote-code --gpu-memory-utilization 0.92 \
  --quantization ascend --no-enable-prefix-caching \
  --speculative-config '{"num_speculative_tokens": 1, "method": "mtp"}' \
  --additional-config '{"scheduler_config":{"recompute_scheduler_enable":true}}' \
  --kv-transfer-config '{"kv_connector":"MooncakeConnectorV1","kv_role":"kv_consumer","kv_port":"37000","kv_connector_extra_config":{"prefill":{"dp_size":1,"tp_size":8},"decode":{"dp_size":1,"tp_size":8}}}' \

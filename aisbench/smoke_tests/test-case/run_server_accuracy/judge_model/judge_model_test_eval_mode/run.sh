...
if [ $? -eq 0 ]
then
    echo "Run ${CASE_NAME} infer test: Success"
else
    echo "Run ${CASE_NAME} infer test: Failed"
    exit $ret_failed
fi
...
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 1 --mode eval --reuse ${TIMESTAMP} 2>&1 | tee ${CUR_DIR}/tmplog.txt

if [ $? -eq 0 ]
then
    echo "Run ${CASE_NAME} eval test: Success"
else
    echo "Run ${CASE_NAME} eval test: Failed"
    exit $ret_failed
fi

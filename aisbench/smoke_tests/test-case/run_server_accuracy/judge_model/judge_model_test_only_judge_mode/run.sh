...
# 同时运行数据集配置
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 1 --mode infer 2>&1 | tee ${CUR_DIR}/tmplog.txt

if [ $? -eq 0 ]
then
    echo "Run ${CASE_NAME} infer process test: Success"
else
    echo "Run ${CASE_NAME} infer process test: Failed"
    exit $ret_failed
fi
...
# 同时运行数据集配置
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 1 --mode judge --reuse ${TIMESTAMP} 2>&1 | tee ${CUR_DIR}/tmplog.txt

if [ $? -eq 0 ]
then
    echo "Run ${CASE_NAME} judge process test: Success"
else
    echo "Run ${CASE_NAME} judge process test: Failed"
    exit $ret_failed
fi

...
do
    ...
    } >> "${AIS_BENCH_CODE_CONFIGS_DIR}/models/vllm_api/${CASE_NAME}_${i}.py"
done
# 添加裁判模型ip端口配置
{
    echo ""
...
# 同时运行数据集配置
ais_bench --models ${CASE_NAME}_0 ${CASE_NAME}_1 --datasets ${CASE_NAME} --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 2 2>&1 | tee ${CUR_DIR}/tmplog.txt
...
model_abbr_list=(vllm-api-generl-chat-0 vllm-api-generl-chat-1)
...
do
    ...
    do
        file_patterns=(
            ...
            "logs/eval/${model_abbr}/${abbr}-${judge_model_abbr}.out"
            "predictions/${model_abbr}/${abbr}.jsonl"
            "predictions/${model_abbr}/${abbr}-${judge_model_abbr}.jsonl"
            "results/${model_abbr}/${abbr}-${judge_model_abbr}.json"
        )

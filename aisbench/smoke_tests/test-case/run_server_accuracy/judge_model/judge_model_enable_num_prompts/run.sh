...
# 同时运行数据集配置
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 1 --num-prompts 5 2>&1 | tee ${CUR_DIR}/tmplog.txt
...
do
    ...
    for pattern in "${jsonl_to_check[@]}"; do
        ...
        if [ "$line_count" -ne 5 ]; then
            echo "Failed. ${file_path} should have 5 lines(equal to --num-prompts), but got $line_count lines"
            exit $ret_failed
        fi
    done
done

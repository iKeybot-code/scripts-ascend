...
# 同时运行数据集配置
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} ${CASE_NAME}_other_ds --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 2 2>&1 | tee ${CUR_DIR}/tmplog.txt
...
# 数据集abbr列表，用于文件检查
judge_dataset_abbr_list=(aime2025_judge)
normal_dataset_abbr_list=(aime2024_normal)

judge_model_abbr="first"

# 检查每个数据集配置的输出文件
for abbr in ${judge_dataset_abbr_list[@]}
do
    file_patterns=(
...
for abbr in ${normal_dataset_abbr_list[@]}
do
    file_patterns=(
        "logs/infer/vllm-api-general-chat/${abbr}.out"
        "logs/eval/vllm-api-general-chat/${abbr}.out"
        "predictions/vllm-api-general-chat/${abbr}.jsonl"
        "results/vllm-api-general-chat/${abbr}.json"
    )

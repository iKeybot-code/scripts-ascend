...
dataset_abbr_list=(aime2024) # 所有数据集配置文件的list
if [ -f ${CUR_DIR}/tmplog.txt ];then
    rm ${CUR_DIR}/tmplog.txt
fi
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME}_0 ${CASE_NAME}_1 --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 2 2>&1 | tee ${CUR_DIR}/tmplog.txt
if [ $? -eq 0 ]
then
...
for pattern in "${jsonl_to_check[@]}"; do
    file_path="${CASE_OUTPUT_PATH}/${TIMESTAMP}/${pattern}"
    # 判断jsonl中刚好只有10行数据
    line_count=$(wc -l < "${file_path}")
    if [ "$line_count" -ne 10 ]; then
        echo "Failed. ${file_path} should have 10 lines(equal to --num-prompts), but got $line_count lines"
        exit $ret_failed
    fi
done
